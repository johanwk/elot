...
=elot-toggle-label-display=.  =F5= is reserved for the user
(see Key Binding Conventions in the Emacs Lisp manual), so
ELOT does not claim it by default.  Discoverability is
preserved through the ELOT easymenu and =M-x
elot-toggle-label-display=.

The tri-state defcustom =elot-bind-f5-toggle-labels= lets the
user opt in to the historical =F5= binding via a one-time
prompt on first =elot-mode= activation.  The answer is
persisted via =customize-save-variable= so the prompt only
appears once.
#+name: src-keybinding
#+begin_src emacs-lisp
