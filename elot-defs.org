                ...
                (dolist (subitem (org-element-contents child))
                  (let ((tag (elot-org-elt-item-tag-str subitem)))
                    (if (elot-meta-annotation-tag-p tag)
                        (progn
                          (when sub-result
                            (push (string-join (nreverse sub-result) "\n") result))
                          (throw 'stop t))
                      (push (string-trim (substring-no-properties (org-element-interpret-data subitem))) sub-result))))
                (when sub-result
                  (push (string-join (nreverse sub-result) "\n") result))))
             (t
              (push (substring-no-properties (org-element-interpret-data child)) result))))))
      (string-join
       (split-string (string-join (nreverse result) "\n") "[ \t]*\n[ \t]*" t)
       "\n")))
