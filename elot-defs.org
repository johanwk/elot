...
**** Hydra interface to tempo templates (RETIRED)
The ELOT hydra has been retired in favour of the easymenu defined
in =elot-mode.el=; see =ELPA-SUBMISSION-PLAN.org=, "Interlude --
Retire the ELOT hydra".  The =F5= binding for label toggling is
retained (see =src-hydra-keybinding= below -- the =defcustom
elot-key-toggle-labels= survives; =elot-key-open-hydra= is removed
in Step H.2).

The former forward-declarations and =defhydra= block are gone.
The =src-tempo-fwd-declare= and =src-hydra-menu= named blocks
have been removed; a re-tangle no longer emits a hydra body.
...
Keybinding, customizable.  The =elot-key-open-hydra= defcustom
and its =local-set-key= entry have been removed as part of Step
H.2 of the Hydra retirement Interlude (see
=ELPA-SUBMISSION-PLAN.org=); only the F5 label-toggle binding
remains.
