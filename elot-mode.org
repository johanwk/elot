...
Load the core ELOT modules (zero external dependencies).  Then
attempt to load the full =elot= package, which brings in SPARQL
execution, LaTeX export filters, OWL import, etc.
The =(require 'elot nil t)= call is /soft/: if =elot.el= (or any of
its external dependencies) is not installed, it silently falls back
to the core.  Every menu item that depends on the full package is
already guarded with =:active (fboundp ...)=, so the menu degrades
gracefully.

=elot-lint= is autoloaded -- it is only pulled in when the user
explicitly calls =elot-org-lint=.
...
#+name: src-elot-buffer-predicate
#+begin_src emacs-lisp
...
** Tempo Templates
:PROPERTIES:
:CUSTOM_ID: tempo-templates
:END:

ELOT uses `tempo-template` to provide skeleton structures for ontologies and resources.
...
Insert a class heading with SKOS annotation properties and completion cookies.
...
#+begin_src emacs-lisp
  (tempo-define-template "elot-table-of-resources"
   ...
   'org-tempo-tags)
#+end_src
** Table to Outline Utility
:PROPERTIES:
:CUSTOM_ID: table-to-outline-utility
:END:
ELOT includes utility functions to generate an outline from an Org table.

#+name: src-mode-table-to-outline
#+begin_src emacs-lisp
...
** SPARQL advice installation
:PROPERTIES:
:CUSTOM_ID: sparql-advice-install
:END:

The ELOT-specific around-advice on =org-babel-execute:sparql= (defined
in =elot-defs.org= block =src-sparql-exec-patch=) is *not* installed at
=require= time -- that would mutate global state for every Emacs session
that merely loads ELOT, which MELPA discourages.  Instead it is
installed by =elot-mode--enable= and removed by =elot-mode--disable=.

A single boolean, =elot--sparql-advice-installed-p=, tracks whether
the advice is currently in place so that multiple ELOT buffers share
a single installation: enabling =elot-mode= in additional buffers is a
no-op for the advice, and disabling =elot-mode= in the *last* such
buffer fully uninstalls it.  The advice body itself still guards on
=elot--is-elot-buffer= as defence in depth.

#+name: src-mode-sparql-advice
#+begin_src emacs-lisp
...
** Xref globals installation
:PROPERTIES:
:CUSTOM_ID: xref-globals-install
:END:

In addition to the SPARQL advice handled above, =elot-mode.el= also
installs a small set of *global* xref customisations: an =:before=
advice on =xref-find-references= (=elot--capture-slurp=, which copies
the current buffer's =elot-slurp= into =elot-slurp-global= so the
=*xref*= buffer can render label overlays), and two entries on
=xref-after-update-hook= (=elot--xref-label-overlay-setup= and
=elot--xref-buffer-enable-backend=).

These were originally installed at top level when =elot-mode.el=
loaded, which mutated global state for every Emacs session that
merely required ELOT.  Following the same pattern as the SPARQL
advice, installation is now driven by the =elot-mode= lifecycle and
reference-counted across ELOT buffers: the first =elot-mode--enable=
installs; subsequent calls only bump the count; =elot-mode--disable=
in the last ELOT buffer fully uninstalls.

The install helper guards each =advice-add= / =add-hook= with
=fboundp= because the backing functions are defined further down in
the file (after the xref backend), so during =require= time the
helpers exist before their targets do.  Installation runs from
=elot-mode--enable= which executes after the file is fully loaded,
so the guards are belt-and-braces rather than strictly required.

#+name: src-mode-xref-globals
#+begin_src emacs-lisp
...
:PROPERTIES:
:CUSTOM_ID: lob-ingest
:END:

=elot-package/elot-lob.org= contains supported, user-facing
=#+call:=-style helpers (=rdfpuml-block=, =kill-prefixes=,
theme/setupfile helpers, =robot-metrics=, =robot-sparql-select=, the
date helpers, etc.).  These named blocks are made available to every
ELOT Org buffer by ingesting the file via =org-babel-lob-ingest=.

Ingestion is a global, idempotent side effect on Org's Library of
Babel: once any ELOT buffer has ingested =elot-lob.org=, every other
Org buffer in the same Emacs session can resolve =#+call:
rdfpuml-block(...)= and friends.  The =elot--lob-ingested-p= flag
ensures we ingest only once per Emacs session, so re-enabling
=elot-mode= in another buffer (or in the same buffer after a toggle)
does not pay the parse cost again.

=locate-library "elot-mode"= resolves to the directory of the loaded
=elot-mode.el=, which sits next to =elot-lob.org= in both a Git
checkout and an installed MELPA package; that gives us a portable
way to find the file without baking in any path.

There is no corresponding teardown in =elot-mode--disable=: ingested
LOB blocks are harmless in non-ELOT buffers, and Org provides no
public API to remove individual entries from the Library of Babel.

#+name: src-mode-lob-ingest
#+begin_src emacs-lisp
...
** Apply user's toggle-labels keybinding

The =defvar elot-mode-map= above runs at /load time/, before the
user's =custom-file= has been read.  Reading
=elot-toggle-labels-key= there would always see the default =nil=
and never install the user's preferred binding.  Instead the
binding is applied (and refreshed) at every =elot-mode--enable=
via the helper below.

A small state variable =elot-mode--applied-toggle-key= tracks the
last key we installed so we can unbind it cleanly if the user
later changes =elot-toggle-labels-key= via Customize.

#+name: src-mode-apply-toggle-key
#+begin_src emacs-lisp
