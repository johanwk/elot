...
- *First select one or more aboutness legs.* Use the table in
  [[*Required constants][Required constants]]. A leg is selected only when its
  stated mode is present. The four legs are independent and complementary:
  choose any non-empty subset, and do not create variable resources merely to
  fill unused legs. Add every resource marked =pattern:action :: CONSTANT=.
...
Add every resource marked =pattern:action :: CONSTANT=. Declarations without
that marker are ontology scaffolding, not pattern inputs. Use this table only to
select the variable instance legs. For each selected leg, assert the defined
class and both relation directions so the result remains informative without
inference.
...
The fixed constants also include ~iof-constr:InformationContentEntity~,
~iof-constr:isAbout~, ~iof-constr:isSubjectOf~, and ~prov:value~. Preserve the
hierarchy declared in this pattern. Do not add source ancestors that the
pattern does not declare. PROV-O defines ~prov:value~ as an
~owl:DatatypeProperty~ that provides a value directly representing an entity.
Reuse this property without importing PROV-O.
...
**** var:TheDenotativeInformation
 ...
 - Facts :: prov:value "denoter value"
   - pattern:value :: xsd:string
 - pattern:action :: MINT
 - pattern:instruction :: A not necessarily unique denoter that distinguishes TheEntity, such as a model name. Assert the defined class, both relation directions, and its value on prov:value. If it uniquely identifies one entity in context, use the designative leg instead.
