...
#+begin_src sparql :url "./IOF-pattern-information-about.omn" :eval never-export :exports results :format ttl :wrap "src ttl" :cache no :post kill-prefixes(data=*this*) :eval never-export
...
- *Use the preferred source for IOF Core.* Make sure to consult =patterns/iof-core.org= as the preferred source for IOF Core terms. This makes a big difference to the quality of the result.
...
- *Mapping Cheat Sheet:*
  - *Unique codes* (VIN, Serial No, UUID, Part ID) ➔ =iof-constr:Identifier= + =iof-constr:designates=
  - *Non-unique names* (Model Name, Brand, Category) ➔ =iof-constr:Denoter= + =iof-constr:denotes=
  - *Characterizations* (Tagline, Condition Report, Summary) ➔ =iof-constr:DescriptiveInformationContentEntity= + =iof-constr:describes=
  - *Rules, Guidance, or Models* (Specification, Manual, Plan, Purpose) ➔ =iof-constr:DirectiveInformationContentEntity= + =iof-constr:prescribes=
...
The value carrier is ~prov:value~, defined by PROV-O as an
~owl:DatatypeProperty~ that provides a value directly representing an
entity. This pattern uses it with literal values.
