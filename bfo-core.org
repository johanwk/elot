...
By default, ELOT will use the /localname/ of an ontology to determine
the filename of an output ontology file. Because the localname of
BFO-2020 is ~bfo.owl~, this will produce ~bfo.owl.omn~ (Manchester Syntax)
and ~bfo.owl.ttl~ (Turtle), which is not intuitive! Therefore, in the
~:PROPERTIES:~ org-mode "drawer" below, below the top headline,
the ~:tangle~ target was changed to ~bfo-core.omn~.
