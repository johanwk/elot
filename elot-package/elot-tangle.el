...
(defun elot-in-ontology-p (&optional pos)
  "Return non-nil when POS (default point) sits inside an ontology heading.
This is the positive test used by readers, writers and lint to
decide whether a heading is ontology source.  It is false inside
any subtree whose `ELOT-context-type' is something else, such as
a pattern tracker, and false outside any ELOT context."
  (save-excursion
    (when pos (goto-char pos))
    (equal (elot-context-type) "ontology")))
...
Checks for `elot-robot-jar-path`."
  ...
  (if (not (and (boundp 'elot-robot-jar-path)
               elot-robot-jar-path
...
Skips conversion when `elot-robot-jar-path' is empty or unset."
  (let* ((omnfile (buffer-file-name))  ;; will run in the tangled buffer
         (omn-p (string-match-p ".omn$" omnfile)))
    (when (and omn-p
              (bound-and-true-p elot-robot-jar-path)
              (not (string-empty-p elot-robot-jar-path)))
...
Creates a dummy root at level 0 to handle multiple top-level ontologies."
  (let* ((dummy-root (list :level 0 :title "ROOT" :descriptions nil :children nil))
         ...
         ;; Level of a heading whose `:ELOT-context-type:' is set to
         ;; something other than "ontology" (for instance a pattern
         ;; tracker).  Such a heading and everything under it is not
         ;; ontology source, so it is skipped entirely.
         (skip-level nil))
    (org-element-map tree 'headline
      (lambda (hl)
        (let ((level (org-element-property :level hl))
              (own-context (org-element-property :ELOT-CONTEXT-TYPE hl)))
          ;; Leave a skipped subtree once we are back at its level or above.
          (when (and skip-level (<= level skip-level))
            (setq skip-level nil))
          (cond
           (skip-level nil)
           ((and own-context (not (equal own-context "ontology")))
            (setq skip-level level)
            nil)
           (t
          ...
          (while (re-search-forward
            ...
            (unless (gethash name seen)
              (let ((hl (org-element-at-point)))
                (when (and (eq (org-element-type hl) 'headline)
                           (= (org-element-property :level hl) 2)
                           ;; Only patch sections that belong to an
                           ;; ontology.  A section heading inside, say, a
                           ;; pattern tracker is not ontology source.
                           (elot-in-ontology-p))
...
Heading nesting is materialised as an inferred \"SubClassOf\" /
\"SubPropertyOf\" attribute naming the nearest declaring ancestor,
mirroring the OMN tangle path (`elot-omn-resource-frame');
`:nodeclare:' grouping headings are transparent.
Returns a list of lists: (URI label (plist of attributes))."
  (let ((stack (list (cons (or hierarchy (bound-and-true-p elot-headline-hierarchy)) nil)))
        (result nil))
    (while stack
      (let* ((entry      (pop stack))
             (node       (car entry))
             (parent-uri (cdr entry))
             (uri        (plist-get node :uri))
             (label      (plist-get node :label))
             (tags       (plist-get node :tags))
             (desc       (plist-get node :descriptions))
             (children   (plist-get node :children))
             (declared   (and uri (stringp uri) (not (member "nodeclare" tags))))
             ;; `:nodeclare:' grouping headings are transparent: their
             ;; children inherit the nearest declaring ancestor as parent.
             (next-parent (if declared uri parent-uri)))
        ...
        (when (and uri (stringp uri))
          (let* ((flat-desc (flatten-tree desc))
            ...
            (let ((attrs (list "rdfs:label" display-label)))
              ...
              ;; Materialise the taxonomy carried by heading nesting, in
              ;; the same shape the OMN tangle path infers it (see
              ;; `elot-omn-resource-frame').  Without this the ELOT label
              ;; DB is blind to SubClassOf / SubPropertyOf for
              ;; Org-ingested sources (`elot-db-supertypes' returns no
              ;; rows), while TTL-ingested sources carry explicit triples.
              (when (and declared parent-uri)
                (let ((key (cond
                            ((equal rdf-type "owl:Class") "SubClassOf")
                            ((and rdf-type (string-match-p "Property$" rdf-type))
                             "SubPropertyOf"))))
                  (when (and key
                             ;; do not duplicate an explicitly authored row
                             (not (let ((p attrs) (found nil))
                                    (while (and p (not found))
                                      (when (and (equal (car p) key)
                                                 (equal (cadr p) parent-uri))
                                        (setq found t))
                                      (setq p (cddr p)))
                                    found)))
                    (setq attrs (nconc attrs (list key parent-uri))))))
        ...
        (when children
          (setq stack (append (mapcar (lambda (c) (cons c next-parent)) children)
                              stack)))))
...
during sync are reported but do not break the local HT population."
  (let ((slurp (elot-build-slurp)))
    ...
    ;; The `elot-db-sync-on-slurp' gate (defined in elot-db.el, but
    ;; consulted defensively via `bound-and-true-p' so this branch
    ;; degrades gracefully if elot-db has not been loaded yet) lets
    ;; test harnesses suppress writes to the user's global cache.
    (when (and slurp
               buffer-file-name
               (fboundp 'sqlite-open)
               ;; Default-true: the variable is nil only when
               ;; explicitly disabled (test-helper.el, etc.).
               (or (not (boundp 'elot-db-sync-on-slurp))
                   (symbol-value 'elot-db-sync-on-slurp)))
      (require 'elot-db nil t)
      (when (fboundp 'elot-db-update-source)
        (condition-case err
            (progn
          ...
          (error
           (message "elot-slurp-to-vars: DB sync failed: %s"
    ...
    (mapconcat (lambda (y)
                 (let* ((key (car y))
                   ...
                   (cond
                    ...
                    ((and (equal key "Rule") meta)
                     (concat ind key ": # WARNING: Rule annotations are not supported in Manchester Syntax\n"
