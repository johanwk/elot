...
# This file is part of ELOT's supported, user-facing API.  It
# collects =#+call:=-style helpers (most notably =rdfpuml-block=,
# =kill-prefixes=, the theme/setupfile helpers, =robot-metrics=,
# =robot-sparql-select=, and the date helpers) that ELOT users
# invoke from their ontology Org files.  =elot-mode= auto-ingests
# this file via =org-babel-lob-ingest= when it is enabled, so the
# named blocks below are available without any manual setup.
#
# The two named blocks =resource-declarations= and
# =resource-taxonomy= that previously lived under "export" have
# been retired together with the legacy LOB pipeline functions in
# =elot.el= (see ELPA-SUBMISSION-PLAN.org Milestone 2).  The new
# tangle pipeline in =elot-tangle.el= produces OMN directly from
# the Org outline and does not need those blocks.
...
#+header: :var prefixes=(elot-prefix-block-from-alist org-link-abbrev-alist-local 'ttl)
