...
Get the tools:
 ...
 4. The ~elot-exporter~  tool converts existing OWL ontologies to ELOT's
    ...
    - the source code is available in [[file:tools/elot-exporter/][=tools/elot-exporter/=]] in this repository.
...
([[file:elot-package/elot-lob.org][=elot-package/elot-lob.org=]])
...
[[file:examples/pets.org][=examples/pets.org=]], showing
...
Two command-line tools are included under the [[file:tools/][=tools/=]] directory:

 - [[file:tools/elot-exporter/README.md][elot-exporter]] --- A Java CLI (OWLAPI + Maven) that converts existing OWL
   ontologies into ELOT's Org-mode format, producing a shaded JAR.

 - [[file:tools/elot-cli/README.md][elot-cli]] --- A TypeScript CLI and VS Code extension that generates OWL
