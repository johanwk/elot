...
DB (`elot-db-get-label' / `elot-db-get-label-any') if available and
not found locally."
...
if available.  Note IDSTRING is a label (not an id) for the local HT
path; the DB fallback treats it as an id, so is only useful when the
lookup is being driven by id."
