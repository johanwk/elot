...
#+begin_src emacs-lisp
  ...
  Obsolete: retained only as a transitional no-op.  Use the ELOT label
  DB (`elot-db-*' / `elot-label-register-source') instead.")
...
#+begin_src emacs-lisp
  ...
  `buffer-file-name', so other buffers can look them up via
  `elot-db-get-label' / `elot-db-get-label-any'.  Errors during
