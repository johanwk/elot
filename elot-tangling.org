...
#+begin_src elisp
(defun elot-tangle-include-outline ()
  "Prepend outline-generated OMN to the current tangled file if it's an .omn file."
  (let* ((target-file (buffer-file-name))
         (is-omn (and target-file (string-match-p "\\.omn$" target-file))))
    (when (and is-omn elot-last-org-source)
      (let ((outline-omn
             (with-current-buffer (find-file-noselect elot-last-org-source)
               (elot-update-headline-hierarchy)
               (let ((nodes (plist-get elot-headline-hierarchy :children))
                     (content ""))
                 (dolist (node nodes)
                   (let ((node-target (plist-get node :tangle-target-omn)))
                     ;; Match target filename (handling potential relative paths)
                     (when (and node-target
                                (string= (file-name-nondirectory target-file)
                                         (file-name-nondirectory node-target)))
                       (setq content (concat content (elot-get-ontology-node-omn node))))))
                 content))))
        (unless (string-empty-p outline-omn)
          (goto-char (point-min))
          (insert outline-omn "\n\n")
          ;; Babel will save the buffer automatically after this hook
          )))))
#+end_src
...
2. ~org-babel-post-tangle-hook~: Add a new function that *prepends* the outline content to the tangled file. This hook runs inside the temporary buffer Babel uses to write the file, so prepending here is safe and efficient.
...
(add-hook 'org-babel-post-tangle-hook #'elot-tangle-include-outline nil t)
#+end_src
