...
Manchester (=omn=), SPARQL, or Turtle syntax.  It originally lived
in =elot.el= (block =src-prefix-blocks= in =elot-defs.org=) and
was relocated here in Milestone 2 Step 2.1 of the ELPA submission
plan: it is needed by =elot-label-display.el= and by the SPARQL
advice in =elot.el=, both of which already =(require 'elot-tangle)=.
