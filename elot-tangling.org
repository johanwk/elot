...
#+end_src
*** Post-Tangle: Merge Outline with User OMN Blocks
The goal is to combine two sources of OMN content into each =.omn= tangle target:

1. *Outline OMN* -- generated programmatically from the AST by =elot-get-ontology-node-omn=
2. *User OMN blocks* -- arbitrary =#+begin_src omn= blocks that =org-babel-tangle= writes

We use =:around= advice on =org-babel-tangle=.  Before babel runs, we
delete stale =.omn= files so we can detect whether babel actually wrote
anything.  After babel runs, we write the outline and append whatever
babel produced.
...
Only activates in buffers where `elot-headline-hierarchy' is set."
  (if (not (bound-and-true-p elot-headline-hierarchy))
      ...
      (apply orig-fn args)
    ;; ELOT buffer: collect targets, delete old files, tangle, then merge
    (let ((omn-targets nil))
      ;; 1. Collect .omn tangle targets and delete stale files
      (dolist (node (plist-get elot-headline-hierarchy :children))
        (when-let ((target (plist-get node :tangle-target-omn)))
          (let ((target-full (expand-file-name target)))
            (push (cons target-full node) omn-targets)
            (when (file-exists-p target-full)
              (delete-file target-full)))))

      ;; 2. Let org-babel-tangle run -- writes user omn blocks (if any)
      (apply orig-fn args)

      ;; 3. For each target, write outline + append user content
      (dolist (entry omn-targets)
        (let* ((target-full (car entry))
               (node        (cdr entry))
...
Advice is added/removed by =elot-mode= via the hook management functions.
No top-level side effects here.
