...
*** Elot helpers for outline and OMN
...
Functions that retrieve ELOT context properties from the Org property
drawer hierarchy.  Used by tempo templates, lint checks, and the old
pipeline.
...
A change to ~elot-org-elt-item-pars-str~: for plain-list sub-items that are *not* meta-annotations, use =buffer-substring-no-properties= with the element's =:begin=/=:end= positions instead of =org-element-interpret-data=. This preserves the original =1.=, =2.=, =3.= numbering verbatim.
This also avoids any other normalization artifacts that =org-element-interpret-data= might introduce (e.g., whitespace changes, markup canonicalization).
#+begin_src emacs-lisp
...
*** Support Tangling and Robot post-processing

**** ROBOT configuration
The ROBOT-related configuration (=elot-robot-jar-path=,
=elot-robot-command-str=) lives in =elot-robot.el= (the shared
process layer used by tangle, sources, and gptel tools).  We simply
require it here so that all existing references in this file keep
working unchanged.  Only the legacy =elot-robot-command= wrapper
(shell-string based) and the =elot-exporter-*= variables remain
tangle-local.
...
**** OMN → Turtle conversion
The three =elot-robot-omn-to-ttl= functions: a synchronous batch
helper, an asynchronous interactive helper, and the main dispatcher.
...
**** Error navigation
Parse ROBOT error output and jump to the offending location in the
OMN file or the originating Org heading.
...
**** Post-tangle hook
Track the source Org file and trigger OMN → Turtle conversion after
tangling completes.
...
This helper returns a textual block of prefix declarations in
Manchester (=omn=), SPARQL, or Turtle syntax.  It lives here
(rather than in =elot.el=) because it is needed by
=elot-label-display.el= and by the SPARQL advice in =elot.el=,
both of which already =(require 'elot-tangle)=.
...
*** "Sanity check" where headings lack property drawers
If the "Prefixes" heading lacks ~:prefixdefs: yes~, then add it before updating. The current Elot doesn't have a default drawer on the Prefixes heading.
#+begin_src emacs-lisp
...
A user can easily forget to add property drawers to resource-declaration headings as well.
#+begin_src emacs-lisp
...
Returns a list of lists: (URI label (plist of attributes))."
  (let ((stack (list (cons (or hierarchy (bound-and-true-p elot-headline-hierarchy)) nil)))
    ...
    (nreverse result)))
#+end_src
*** Variables for slurp data
These buffer-local variables hold the parsed entity data (populated by
=elot-slurp-to-vars= below).  They are used by =elot-label-display.el=
for fontification and lookup.
...
*** Helper functions
Note, in ~elot-tangle-buffer-to-omn~ we save the value of ~org-link-abbrev-alist-local~ before exporting each ontology. The exporting happens in a temporary buffer where ~org-link-abbrev-alist-local~ isn't set, and without it ~elot-annotation-string-or-uri~ can't interpret URI-looking annotation values as URIs.

This is a temporary solution only. The list of prefixes to be applied should be properly handled /per ontology/, which the following doesn't. TODO.

Macro definitions are carried into the temp file with variable ~omn-save~. This gives some strange results for the ~modification-time~ macro. TODO find out why.
#+begin_src emacs-lisp
